...
class GetBatchDbAdapterTest extends AbstractIntegrationTest {
    ...
    @Autowired
    private GetBatchDbAdapter getBatchDbAdapter;

    @Test
    @DataSet("datasets/infrastructure/output/db/batch/get_batch_setup.yml")
    void get_success() {
        final var actual = getBatchDbAdapter.get(BATCH_ID);
    ...
    @DataSet("datasets/infrastructure/output/db/batch/get_batch_setup.yml")
    void get_notFound() {
        assertThrows(ResourceNotFoundException.class, () -> getBatchDbAdapter.get(WRONG_BATCH_ID));
    }
}